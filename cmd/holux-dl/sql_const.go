...
var (
	...
	CREATE_TABLE = map[string]string{
		"trackpoints":        `CREATE TABLE trackpoints ( track     INTEGER NOT NULL, time      TIMESTAMP NOT NULL, latitude  REAL NOT NULL, longitude REAL NOT NULL, "elevation/m" REAL NOT NULL,  "height/m"    REAL NOT NULL,  "heartrate/bpm" INTEGER, "cadence/rpm"   INTEGER, unknown TEXT, FOREIGN KEY(track) REFERENCES tracks(id) ON DELETE CASCADE );`,
		...
		"tracks":             `CREATE TABLE tracks ( id   INTEGER PRIMARY KEY, time TIMESTAMP NOT NULL, name TEXT,  "distance/m" INTEGER NOT NULL, "duration/s" INTEGER NOT NULL, "ascent/m"  INTEGER NOT NULL, "descent/m" INTEGER NOT NULL, unknown TEXT );`,
	}
	INSERT = map[string]string{
		"POI":        `INSERT INTO points_of_interest(track, time, latitude, longitude) VALUES (?, ?, ?, ?);`,
		"trackpoint": `INSERT INTO trackpoints VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);`,
		"track":      `INSERT INTO tracks(time, name, "distance/m", "duration/s",  "ascent/m", "descent/m", unknown) VALUES (?, ?, ?, ?, ?, ?, ?);`,
	}
)
